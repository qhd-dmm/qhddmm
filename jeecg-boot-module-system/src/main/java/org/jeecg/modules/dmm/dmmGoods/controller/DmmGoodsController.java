package org.jeecg.modules.dmm.dmmGoods.controller;
...
import org.jeecg.modules.dmm.dmmGoods.entity.DmmGoods;
import org.jeecg.modules.dmm.dmmGoods.service.IDmmGoodsService;
...
@Api("商品表")
@RestController
@RequestMapping("/dmmGoods/dmmGoods")
public class DmmGoodsController {
	...
	public Result<IPage<DmmGoods>> queryPageList(DmmGoods dmmGoods,
		...
		QueryWrapper<DmmGoods> queryWrapper = QueryGenerator.initQueryWrapper(dmmGoods, req.getParameterMap());
	...
	public Result<DmmGoods> edit(@RequestBody DmmGoods dmmGoods) {
		Result<DmmGoods> result = new Result<DmmGoods>();
		DmmGoods dmmGoodsEntity = dmmGoodsService.getById(dmmGoods.getId());
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<DmmGoods> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              DmmGoods dmmGoods = JSON.parseObject(deString, DmmGoods.class);
              queryWrapper = QueryGenerator.initQueryWrapper(dmmGoods, request.getParameterMap());
          }
      ...
      List<DmmGoods> pageList = dmmGoodsService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "商品表列表");
      mv.addObject(NormalExcelConstants.CLASS, DmmGoods.class);
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<DmmGoods> listDmmGoodss = ExcelImportUtil.importExcel(file.getInputStream(), DmmGoods.class, params);
              for (DmmGoods dmmGoodsExcel : listDmmGoodss) {
                  dmmGoodsService.save(dmmGoodsExcel);
              }
              return Result.ok("文件导入成功！数据行数:" + listDmmGoodss.size());
          } catch (Exception e) {
              log.error(e.getMessage(),e);
