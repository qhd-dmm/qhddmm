package org.jeecg.modules.dmm.dmmGoodsCategory.controller;
...
import org.jeecg.modules.dmm.dmmGoodsCategory.entity.DmmGoodsCategory;
import org.jeecg.modules.dmm.dmmGoodsCategory.service.IDmmGoodsCategoryService;
...
@Api("商品分类表")
@RestController
@RequestMapping("/dmmGoodsCategory/dmmGoodsCategory")
public class DmmGoodsCategoryController {
	...
	public Result<IPage<DmmGoodsCategory>> queryPageList(DmmGoodsCategory dmmGoodsCategory,
		...
		QueryWrapper<DmmGoodsCategory> queryWrapper = QueryGenerator.initQueryWrapper(dmmGoodsCategory, req.getParameterMap());
	 ...
	 * @return
	 */
	@AutoLog(value = "商品分类表-编辑")
	@ApiOperation(value="商品分类表-编辑", notes="商品分类表-编辑")
	@PutMapping(value = "/edit")
	public Result<DmmGoodsCategory> edit(@RequestBody DmmGoodsCategory dmmGoodsCategory) {
		Result<DmmGoodsCategory> result = new Result<DmmGoodsCategory>();
		DmmGoodsCategory dmmGoodsCategoryEntity = dmmGoodsCategoryService.getById(dmmGoodsCategory.getId());
	 ...
	 * @return
	 */
	@AutoLog(value = "商品分类表-通过id查询")
	@ApiOperation(value="商品分类表-通过id查询", notes="商品分类表-通过id查询")
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<DmmGoodsCategory> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              DmmGoodsCategory dmmGoodsCategory = JSON.parseObject(deString, DmmGoodsCategory.class);
              queryWrapper = QueryGenerator.initQueryWrapper(dmmGoodsCategory, request.getParameterMap());
          }
      ...
      List<DmmGoodsCategory> pageList = dmmGoodsCategoryService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "商品分类表列表");
      mv.addObject(NormalExcelConstants.CLASS, DmmGoodsCategory.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("商品分类表列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<DmmGoodsCategory> listDmmGoodsCategorys = ExcelImportUtil.importExcel(file.getInputStream(), DmmGoodsCategory.class, params);
              for (DmmGoodsCategory dmmGoodsCategoryExcel : listDmmGoodsCategorys) {
                  dmmGoodsCategoryService.save(dmmGoodsCategoryExcel);
              }
              return Result.ok("文件导入成功！数据行数:" + listDmmGoodsCategorys.size());
          } catch (Exception e) {
              log.error(e.getMessage(),e);
